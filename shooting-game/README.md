...
Run all commands from this folder (`shooting-game/`).
...
Connect the repo, build command `cd shooting-game && npm install && npm run build`, output directory `shooting-game/dist`, production branch `main`. (Or set the project root to `shooting-game` in the dashboard if your host supports it.)
